...
p = linspace(14.7,5000,300).*psia; p=p';
...
figure(1)
%plot(p, Zg_OMO,'g', p, Z_V, 'b--',p, Z_g_CMG,'r*')
plot(p, Zg_OMO,'g', p, Z_V, 'b--')
% plot(p, Z_V2,'r', p, Z_V, 'b--')
title('Comparison of Z-factor Models')
xlabel('Pressure, psi')
ylabel('Z-factor')
legend({'Z_g OMO','Z_g MRST','Z_g CMG',})

figure(2)
%plot(p, viscLBC1,'g', p, viscLee, 'b--', p, mu_MRST,p, visc_CMG,'r*')
plot(p, viscLBC1,'g', p, viscLee, 'b--', p, mu_MRST)
title('Comparison of Viscosity Models')
xlabel('Pressure, psi')
ylabel('Viscosity, cp')
legend({'visc LBC','visc Lee','visc MRST','visc CMG'})