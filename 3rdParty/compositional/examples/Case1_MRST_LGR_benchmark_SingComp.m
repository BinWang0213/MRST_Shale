...
[poro_rock,poro_frac]=deal(0.07,0.999);
[perm_rock,perm_frac]=deal(0.0005*milli*darcy,10*darcy);
...
surface_density=0.678876738081682;
...
flowfluid = initSimpleADIFluid('n', [nkr, nkr, nkr], 'rho', [1000, 800, surface_density]);
...
ws = {ws_comp, ws_BO};
shortname = {'comp', 'BO'};
plotWellSols(ws, cumsum(schedule.step.val), 'datasetnames', names,'field','qGs')
...
% t_Sorb_NoSorb = csvread('CMG_10000mdft_LGR_excel_scratch_0.csv',1,0);
% figure(4)
% plot(t_Sorb_NoSorb(:,1),t_Sorb_NoSorb(:,3),'k*',...
%     cumsum(schedule.step.val)./86400,Compositional,'r',...
%     cumsum(schedule.step.val)./86400,BlackOil,'b--')
% legend({'CMG','Compositional','BlackOil'})
% 
% error = (Compositional(end)-t_Sorb_NoSorb(end,3))/t_Sorb_NoSorb(end,3)