classdef GilmanKazemiOilWaterGasTransferFunction < TransferFunction
    % Gilman Kazemi 1988 3 Phase Transfer Function
    ...
    methods
        function transferfunction = GilmanKazemiOilWaterGasTransferFunction(shape_factor_name,block_dimension)
        ...
        function [Talpha] = calculate_transfer(ktf,model,fracture_fields,matrix_fields)
            ...
            hof=(sO-sorf)./(1-swcf-sorf)*lz;
            hwf=(sW-swcf)./(1-swcf-sorf)*lz;
            hgf=(sG-sgrf)./(1-sgrf-swcf-sorf);
            
            hom=(som-sorm)./(1-sorm-swcm)*lz;
            hwm=(swm-swcm)./(1-sorm-swcm)*lz;
            hgm=(sG-sgrm)./(1-sgrm-swcm-sorm);
            ...
            tg=(sigma.*bGm.*krgt./mugm).*(pG-pgm-rhoGf.*g.*(hgf-Df)-rhoGm.*g.*(hgm-Dm)); 
