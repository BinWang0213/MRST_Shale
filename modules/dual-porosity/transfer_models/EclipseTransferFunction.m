classdef EclipseTransferFunction < TransferFunction
    %% This is the transfer function defined in Eclipse simulator. This is a two-phase
    % version.
    properties
        shape_factor_object
        shape_factor_name
    ...
    methods
        ...
        function transferfunction = EclipseTransferFunction(shape_factor_name,block_dimension)
        ...
        function [Talpha] = calculate_transfer(ktf,model,fracture_fields,matrix_fields)
            ...
            hwf = ((sW-swrf) ./(1-snrf-swrf)).*lz;
            hwm = ((swm-swrm)./(1-snrm-swrm)).*lz;
            hnf = lz-hwf;
            hnm = lz-hwm;
            ...
            psi_of = pO + rhon*g*hnf;
            psi_wf = pW + rhow*g*hwf;
            ...
            tw=(sigma.*bWm.*krwt./muwm).*( pW-pwm + 0.5*g.*delta_rho.*(hwf-hwm));
            to=(sigma.*bOm.*krot./muom).*( pO-pom - 0.5*g.*delta_rho.*(hwf-hwm));
